...
fetch('https://jsonplaceholder.typicode.com/users')
    ...
    .then(users => {
            for (const user of users) {
                    ...
                    divka.innerText=`${user.id}--${user.name}`
                    divka.className='divka'
                    const knopkaPerehod = document.createElement('button');
                    knopkaPerehod.innerText='Perehid na user-detail';
                    box.appendChild(divka);
                    box.appendChild(knopkaPerehod);
                    conteiner.appendChild(box)
                    knopkaPerehod.onclick=()=>{
                            location.href=`./user-details.html?dani=${JSON.stringify(user)}`
                    }
